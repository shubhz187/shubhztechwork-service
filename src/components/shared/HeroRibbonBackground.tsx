...
export const HeroRibbonBackground: React.FC<HeroRibbonBackgroundProps> = ({ idPrefix }) => {
  ...
  // FPS-independent cycle: always 10 seconds regardless of fps
  const cycle = fps * 10;
    ...
    >
      ...
      >
        <defs>
          ...
          {/* Radial gradient for soft glow spots (replaces expensive blur filters) */}
          <radialGradient id={id('spotBlue')}>
            <stop offset="0%" stopColor={BLUE_BRIGHT} stopOpacity="0.06" />
            <stop offset="100%" stopColor={BLUE_BRIGHT} stopOpacity="0" />
          </radialGradient>
          <radialGradient id={id('spotCoral')}>
            <stop offset="0%" stopColor={CORAL} stopOpacity="0.05" />
            <stop offset="100%" stopColor={CORAL} stopOpacity="0" />
          </radialGradient>
          <radialGradient id={id('spotTeal')}>
        ...
        <path d={r1Edge} fill="none" stroke={`url(#${id('r1Stroke')})`} strokeWidth="2.5" />
        <path d={r1Edge2} fill="none" stroke={`url(#${id('r1Inner')})`} strokeWidth="1.5" />

        {/* ══════ Ribbon 2: Lower-left ══════ */}
        <path d={r2Edge} fill="none" stroke={CORAL} strokeWidth="35" opacity="0.02" />
        <path d={r2Fill} fill={`url(#${id('r2Fill')})`} />
        <path d={r2Edge} fill="none" stroke={`url(#${id('r2Stroke')})`} strokeWidth="2.5" />
        <path d={r2Edge2} fill="none" stroke={`url(#${id('r1Inner')})`} strokeWidth="1" />

        {/* ══════ Ribbon 3: Middle accent ══════ */}
        <path d={r3Edge} fill="none" stroke={`url(#${id('r3Stroke')})`} strokeWidth="1" />

        {/* ══════ Ribbon 4: Upper-left accent ══════ */}
        {!isMobile && (
          <path d={r4Edge} fill="none" stroke={`url(#${id('r4Stroke')})`} strokeWidth="0.8" />
        )}
